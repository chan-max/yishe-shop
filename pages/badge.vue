<template>
  <div class="badge-page">
    <header class="bd-navbar">
      <div class="navbar-inner">
        ...
        <nav class="navbar-links">
          <a href="#features">马口铁防锈防刮</a>
          <a href="#details">磨砂/镭射膜</a>
          <a href="#products">专属商品</a>
          <NuxtLink to="/design" class="navbar-cta">定制徽章</NuxtLink>
        </nav>
      </div>
    </header>
    ...
    <section class="bd-hero">
      <div class="hero-container">
        <div class="hero-content animate-up">
          ...
          <h1 class="hero-title">
            佩戴态度与二次元<br>
            <span class="title-highlight">定制痛包马口铁徽章</span>
          </h1>

          <p class="hero-subtitle">
            支持 44mm / 58mm / 75mm 经典圆章与心形、方形徽章定制。加厚加硬马口铁包边防锈底座，覆镭射闪粉膜与哑光砂感膜，背包扎包专属谷子。
          </p>

          <div class="hero-actions">
            <NuxtLink to="/design" class="btn-purple">
              开始徽章 POD 定制
            </NuxtLink>
        ...
        <div class="hero-mockup-wrap animate-up" style="animation-delay: 0.15s">
          <div class="mockup-card">
            <svg class="badge-svg" viewBox="0 0 300 320" fill="none">
              <!-- Pin Badge Circle -->
              <circle cx="150" cy="150" r="100" fill="#8B5CF6" stroke="#5B21B6" stroke-width="4" />
              <!-- Inner Rim Detail -->
              <circle cx="150" cy="150" r="90" fill="none" stroke="#C4B5FD" stroke-width="2" stroke-dasharray="6 6" />
              <!-- Badge Pin back representation -->
              <path d="M110 110 L190 190" stroke="#DDD6FE" stroke-width="6" stroke-linecap="round" />
              <circle cx="150" cy="150" r="40" fill="#06B6D4" opacity="0.9" />
              <text x="150" y="155" font-family="sans-serif" font-size="12" font-weight="900" fill="#FFF" text-anchor="middle">BADGE</text>
            </svg>
            <div class="mockup-tag tag-top">44/58/75mm 多尺寸</div>
            <div class="mockup-tag tag-bottom">防锈马口铁 · 覆膜闪粉</div>
          </div>
        </div>
      </div>
    </section>
    ...
    <!-- Products -->
    <section id="products" class="bd-products">
      <div class="section-container">
        <div class="section-header animate-up">
          <span class="section-badge">专属商品</span>
          <h2 class="section-title">【徽章】在售款式</h2>
          <p class="section-sub">实时展示分类为「徽章」的最新已上架商品。</p>
        </div>
          ...
          >
            ...
            <div class="product-details">
              ...
              <div class="product-bottom">
                <span class="product-price">¥{{ item.price || '8.00' }}</span>
        ...
        <div v-else class="empty-box animate-up">
          ...
          <h3>暂无【徽章】分类在售商品</h3>
          <p>管理员可在后台发布分类为「徽章」的商品。您现在可以直接进入 POD 设计器自定图案！</p>
          <NuxtLink to="/design" class="btn-purple">进入 POD 设计器</NuxtLink>
        </div>
      </div>
    </section>
    ...
    <footer class="bd-footer">
      <div class="footer-container">
        <span class="footer-brand">衣设 yishe · 徽章专属介绍页</span>
...
useSeoMeta({
  title: '马口铁徽章定制 · 痛包周边胸章 · 衣设 yishe',
  ogTitle: '马口铁徽章定制 · 痛包周边胸章 · 衣设 yishe',
  description: '马口铁徽章/胸章定制，44mm/58mm/75mm 多尺寸可选，覆镭射闪粉/哑光膜，二次元扎包痛包谷子必选。',
  keywords: '徽章定制, POD徽章, 马口铁胸章, 痛包徽章, 二次元谷子定制'
})
...
onMounted(async () => {
  ...
  loading.value = true
  try {
    const res = await fetchPublishedProducts({ type: '徽章', pageSize: 20 })
    products.value = res || []
  } catch (e) {
    console.error(e)
  } finally {
...
<style scoped>
@import url('https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@700;800&family=Plus+Jakarta+Sans:wght@500;700&display=swap');
...
.bd-navbar { position: sticky; top: 0; z-index: 100; background: rgba(245, 243, 255, 0.9); backdrop-filter: blur(16px); border-bottom: 1px solid var(--border-color); padding: 0 24px; }
...
.brand-icon { width: 22px; height: 22px; stroke: var(--accent-purple); }
...
.navbar-cta { background: var(--accent-purple); color: #FFF !important; font-weight: 700; padding: 8px 18px; border-radius: 8px; }

.bd-hero { padding: 80px 24px; }
...
.badge-tag { display: inline-flex; align-items: center; gap: 8px; background: rgba(139, 92, 246, 0.1); color: var(--accent-purple); padding: 6px 14px; border-radius: 9999px; font-size: 13px; font-weight: 700; margin-bottom: 24px; }
.tag-icon { width: 14px; height: 14px; stroke: var(--accent-purple); }
.hero-title { font-family: 'Space Grotesk', sans-serif; font-size: clamp(2.4rem, 4.5vw, 3.8rem); font-weight: 800; line-height: 1.2; margin-bottom: 20px; }
.title-highlight { color: var(--accent-purple); }
.hero-subtitle { font-size: 1.05rem; color: var(--text-muted); line-height: 1.7; margin-bottom: 32px; max-width: 520px; }

.hero-actions { display: flex; gap: 14px; }
.btn-purple { background: var(--accent-purple); color: #FFF; font-weight: 700; padding: 13px 28px; border-radius: 10px; text-decoration: none; }
...
.badge-svg { width: 260px; height: 270px; }
...
.bd-products { padding: 80px 24px; }
.section-container { max-width: 1200px; margin: 0 auto; }
.section-header { text-align: center; margin-bottom: 48px; }
.section-badge { display: inline-block; color: var(--accent-purple); font-size: 11px; font-weight: 800; letter-spacing: 0.1em; text-transform: uppercase; margin-bottom: 8px; }
.section-title { font-family: 'Space Grotesk', sans-serif; font-size: 2rem; font-weight: 800; margin-bottom: 8px; }
...
.product-img-box { position: relative; width: 100%; padding-top: 100%; background: #F5F3FF; }
.product-img-box img { position: absolute; inset: 0; width: 100%; height: 100%; object-fit: cover; }
.product-cat-tag { position: absolute; top: 12px; left: 12px; background: var(--accent-purple); color: #FFF; font-size: 11px; font-weight: 700; padding: 2px 8px; border-radius: 4px; }
...
.product-price { font-size: 18px; font-weight: 800; color: var(--accent-purple); }
...
.empty-icon-wrap { width: 56px; height: 56px; background: rgba(139, 92, 246, 0.1); border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 16px; }
.empty-icon-wrap svg { width: 28px; height: 28px; stroke: var(--accent-purple); }
...
.bd-footer { padding: 28px 24px; background: #FFF; border-top: 1px solid var(--border-color); }
...
.footer-link { color: var(--accent-purple); text-decoration: none; font-weight: 600; }
